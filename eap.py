...
def encode_EAP(E):
    ...
    if E.type == EAP_TYPE_IDENTITY:
        E.len = 4+len(E.msg)/2
        ret = ("%02X" % E.cmd).encode() + ("%02X" % E.id).encode() + ("%04X" % int(E.len)).encode()
        ret = ret + E.msg
        dbg = "EAP-Payload", ret
        logging.debug(dbg)
        return ret
...
def addEAPIdentity(msg):
    return ("%02X" % EAP_TYPE_IDENTITY).encode() + codecs.encode(msg.encode(), "hex")
...
def exec_calc(cmd_type,params):
    ...
    if platform.system()=="SunOS":
        ret=subprocess.getoutput("./eapcalc.solx86"+" "+args)
    ...
    if platform.system()=="Linux":
        ret=subprocess.getoutput("./eapcalc.linux"+" "+args)
    dbg="Calc output",ret
    logging.debug(dbg)
