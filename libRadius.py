...
def dictVENDORcode2id(code):
    logging.debug(f"Searching Vendor dictionary for C{code}")
    ...
    bailOut(f"Searching Vendor dictionary failed for C{code}")

# Find Vendor definition in dictionary: TGPP->10415
def dictVENDORid2code(vendor_id):
    logging.debug(f"Searching Vendor dictionary for V{vendor_id}")
    for vendor in dict_vendors:
    ...
    bailOut(f"Searching Vendor dictionary failed for V{vendor_id}")
...
def dictCOMMANDname2code(name):
    ...
    bailOut(f"Searching CMD dictionary failed for N{name}")
...
def decode_TPasswd(data,dlen):
    logging.info(data, dlen, len(data))
    (tag, data) = chop_msg(data, 2)
...
def encode_finish(A, pktlen, data):
    ...
    logging.debug(f"Packing C: {A.code} V: {A.vendor} L: {pktlen} D: {ret}")
    ret = ("%02X" % int(A.code)).encode() + ("%02X" % int(pktlen)).encode() + ret
    if A.vendor != 0:
        ret = ("%02X" % 26).encode() + ("%02X" % (6 + len(ret)/2)).encode() + ("%08X" % int(A.vendor)).encode() + ret
    logging.info(f'ret:{ret}')
    return ret

def encode_OctetString(A, data):
    logging.info(f"encode_octet {A.type} {data}")
    fs = "!" + str(len(data)) + "s"
    logging.debug(f"Encoding String format: {fs}")
    ret = codecs.encode(struct.pack(fs, data.encode()), encoding='hex')
    logging.info(f'encode struct pack data:{ret}')
    pktlen = 2 + len(ret) / 2
    return encode_finish(A, pktlen, ret)

def encode_UTF8String(A,data):
    utf8data = codecs.encode(data[0], "utf-8")
    fs = "!" + str(len(utf8data)) + "s"
    logging.debug(f"Encoding UTF8 {utf8data} L {len(utf8data)} F {fs}")
    ret = codecs.encode(struct.pack(fs, utf8data), encoding='hex')
    pktlen = 2 + len(ret) / 2
    return encode_finish(A, pktlen, ret)

def encode_Integer32(A, data):
    ret = codecs.encode(struct.pack("!I", int(data)), encoding='hex')
    pktlen = 6
    return encode_finish(A, pktlen, ret)
...
def encode_Float32(A, data):
    ret = codecs.encode(struct.pack("!f", data), encoding='hex')
    pktlen=6
    return encode_finish(A, pktlen, ret)

def encode_Integer64(A, data):
    ret = codecs.encode(struct.pack("!Q", data), encoding='hex')
    pktlen=8
    return encode_finish(A, pktlen, ret)
...
def encode_Float64(A, data):
    ret = codecs.encode(struct.pack("!d", data), "hex_codec")
    pktlen = 10
    return encode_finish(A, pktlen, ret)

def encode_Address(A, data):
    ret = codecs.encode(pack_address(data), encoding='hex')
    pktlen = 2 + len(ret)/2
    return encode_finish(A, pktlen, ret)
...
def encode_TString(A, data):
    ...
    logging.debug(dbg)
    ret = codecs.encode(struct.pack(fs, data), encoding='hex')
    pktlen = 2 + len(ret)/2
    return encode_finish(A, pktlen, ret)
...
def do_encode(A, data):
    logging.info(f"do_encode: {A.type} {data}")
    if A.type in asUTF8:
...
def getAVPDef(AVP_Name, AVP_Value):
    ...
    logging.info(f"A:{A.name} {A.code} {A.type}")
    if A.name == "":
...
def encodeAVP(AVP_Name, AVP_Value):
    ...
    if type(AVP_Value).__name__ == 'list':
        ...
        for x in AVP_Value:
            p = p + x
        msg = getAVPDef(AVP_Name, codecs.decode(p, "hex_codec"))
    else:
...
def createPacket(H):
    ...
    ret = ("%02X" % H.Code).encode() + ("%02X" % int(H.Identifier)).encode() + ("%04X" % int(H.len)).encode()
    ret = ret + codecs.encode(H.Authenticator.encode('utf-8'), encoding='hex') + H.msg
    logging.debug(f"Header fields C {H.Code} I {H.Identifier} L {H.len} A {H.Authenticator}")
    logging.debug(f"Radius hdr+data:{ret}")
    return ret
...
def createRes(H, auth, avps, secret):
    H.msg = joinAVPs(avps)
...
def PwCrypt(password, authenticator, secret):
  ...
  #  password = password.encode('utf-8')
    ...
    logging.info(f'auth:{authenticator}')
    logging.info(f's+a:{secret + authenticator}')
    logging.info(f"s+a encoded:{secret.encode('utf-8') + authenticator.encode('utf-8')}")
    hash = md5_constructor(secret.encode() + authenticator.encode()).digest()
    result = ''
    last = authenticator
    while buf != '':
        hash = md5_constructor(secret.encode() + last.encode()).digest()
        logging.info(f"{len(hash)} - {len(buf)}")
        for i in range(16):
            result += chr(ord(str(hash)[i]) ^ ord(str(buf)[i]))
        last = result[-16:]
        buf = buf[16:]
    return result
