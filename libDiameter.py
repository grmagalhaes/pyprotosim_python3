...
def decode_Integer32(data):
    ret = struct.unpack("!I", codecs.decode(data, 'hex'))[0]
    return int(ret)

def decode_Integer64(data):
    ret = struct.unpack("!Q", codecs.decode(data, 'hex'))[0]
    return int(ret)
...
def decode_Float32(data):
    ret=struct.unpack("!f", codecs.decode(data, 'hex'))[0]
    return ret

def decode_Float64(data):
    ret=struct.unpack("!d", codecs.decode(data, 'hex'))[0]
    return ret
    
def decode_Address(data):
    if len(data) <= 16:
        data = data[4:12]
        ret = inet_ntop(socket.AF_INET, codecs.decode(data, 'hex'))
    else:
        data = data[4:36]
        ret = inet_ntop(socket.AF_INET6, codecs.decode(data, 'hex'))
    return ret
...
def decode_OctetString(data,dlen):
    ...
    logging.debug(dbg)
    ret = struct.unpack(fs, codecs.decode(data, 'hex')[0:dlen-8])[0]
    return ret
...
def encode_finish(A, flags, pktlen, data):
    ...
    logging.debug(dbg)
    ret = ("%08X" % int(A.code)).encode() + ("%02X" % int(flags)).encode() + ("%06X" % int(pktlen)).encode() + ret
    return ret
...
def encode_UTF8String(A, flags, data):
    ...
    logging.debug(dbg)
    ret = codecs.encode(struct.pack(fs, utf8data), 'hex)')
    pktlen = 8 + len(ret)/2
    return encode_finish(A, flags, pktlen, ret)
    
def encode_Integer32(A, flags, data):
    r = struct.pack("!I", data)
    ret = codecs.encode(r, 'hex')
    pktlen = 12
    return encode_finish(A, flags, pktlen, ret)
...
def encode_Float32(A,flags,data):
    ret = codecs.encode(struct.pack("!f", data), "hex")
    pktlen = 12
    return encode_finish(A, flags, pktlen, ret)
    
def encode_Integer64(A,flags,data):
    ret = codecs.encode(struct.pack("!Q", data), "hex")
    pktlen = 16
    return encode_finish(A, flags, pktlen, ret)
...
def encode_Float64(A, flags,data):
    ret = codecs.encode(struct.pack("!d", data), "hex")
    pktlen = 16
    return encode_finish(A,flags,pktlen,ret)

def encode_Address(A,flags,data):
    ret = codecs.encode(pack_address(data), "hex")
    pktlen = 8 + len(ret)/2
    return encode_finish(A, flags, pktlen, ret)
    
def encode_IP(A,flags,data):
    ret = codecs.encode(pack_address(data), "hex")[4:]
    pktlen = 8 + len(ret)/2
    return encode_finish(A, flags, pktlen, ret)
    ...
    if type(AVP_Value).__name__ == 'list':
        ...
        for x in AVP_Value:
            while len(x)/2 < calc_padding(len(x)/2):
                x = x + '00'
            p = p + x
        msg = getAVPDef(AVP_Name, codecs.decode(p, 'hex'))
    else:
...
def decodeAVP(msg):
    ...
    logging.debug(dbg)
    mcode = struct.unpack("!I", codecs.decode(scode, 'hex'))[0]
    mflags = ord(codecs.decode(sflag, "hex"))
    data_len = struct.unpack("!I", "\00" + codecs.decode(slen, 'hex'))[0]
    mvid = 0
    if mflags & DIAMETER_FLAG_VENDOR:
        (svid, msg) = chop_msg(msg, 8)
        mvid = struct.unpack("!I", codecs.decode(svid, "hex"))[0]
        data_len -= 4
    A = AVPItem()
...
def createRes(H, avps):
    ...
    H.len = len(data)/2+20
    ret = ("01"+"%06X" % int(H.len)).encode() + ("%02X" % int(H.flags)).encode() + ("%06X" % int(H.cmd)).encode()
    ret = ret + ("%08X" % H.appId).encode() + ("%08X" % H.HopByHop).encode() + ("%08X" % H.EndToEnd).encode() + data
...
# Set Hop-by-Hop and End-to-End fields to same values
...
def splitMsgAVPs(msg):
    ...
    while len(msg) != 0:
      slen = "00" + msg[10:16]
      mlen = struct.unpack("!I", codecs.decode(slen, "hex"))[0]
